...
function Marquee({ imagesurl, direction }) {
    return (
        <div className='flex w-full overflow-hidden'>
            ...
            <motion.div initial={{ x: direction === "right" ? "-100%" : "0" }} animate={{ x: direction === "right" ? "0%" : "-100%" }} transition={{ ease: "linear", duration: 20, repeat: Infinity }} className='flex flex-shrink-0 gap-40 py-10 pr-40'>
                {imagesurl.map((url, index) => <img key={index} src={url} className='w-[6vw] flex-shrink-0' />)}
            </motion.div>
